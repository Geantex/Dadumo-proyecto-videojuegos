...
namespace Map
{
    public static class MapGenerator
    {
        ...
        private static readonly List<NodeType> RandomNodesEarly = new List<NodeType>
        ...
        private static NodeType GetRandomNode(int queLayer)
        {
            if(queLayer < 3){
                return RandomNodesEarly[Random.Range(0, RandomNodesEarly.Count)];
            }
            else{
                NodeType chosenNode = new NodeType();
                chosenNode = RandomNodes[Random.Range(0, RandomNodes.Count)];
                if (chosenNode == NodeType.Store) {
                    // de 0 a 1; si sale 0, nos quedamos con Store, si sale 1, hacemos reroll!
